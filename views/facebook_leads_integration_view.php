		...
		<script>
			...
			function subscribeApp(page_id, page_access_token) {
				...
				FB.api('/' + page_id + '/subscribed_apps',
					...
					function(response) {
						...
						var unsubscribe = '<?php echo htmlspecialchars(_l('unsubscribe')); ?>';
			...
			function unsubscribeApps(page_id, page_access_token) {
				...
				FB.api(
					...
					function(response) {
						...
						var subscribe = '<?php echo htmlspecialchars(_l('subscribe')); ?>';
			...
			function checkLoginState() {
				...
				FB.login(function(response) {
					...
					scope: 'manage_pages,publish_pages,pages_show_list,leads_retrieval'
				});
				...
				if(get_option('facebook_pages'))
				{
					...
					foreach ($pages as $page) {
						if (in_array($page->id . get_option('appId'), json_decode(get_option('subscribed_pages')))) {
							$html .= '<tr> <td>' . $page->name . '</td> <td><input type="button" value="' . _l('unsubscribe') . '" id="' . $page->id . '" onclick="unsubscribeApps (' . $page->id . ',\'' . $page->access_token . '\');" class="btn btn-danger"></td> </tr>';
							$flag=1;
						} 
					}
					if($flag==0)
